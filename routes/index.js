...
router.get("/api/content", function (req, res, next) {
    connection.query(`select * from glzhidu b inner join productsort a on a.id = b.leixing ${req.query.keywords ? `where b.sfxs = 0 and a.parid = 215 and (b.bianh like ? or b.biaoti like ?)` : ''}`, [req.query.keywords,req.query.keywords], function (error, rows, fields) {
        if (error || !rows) { res.status(200).send({ status: 'failed' }); return; }
        res.setHeader('Access-Control-Allow-Origin', '*')
        res.status(200).json(rows);
        // res.status(200).send({ results: products, status: 'success' });
